...
if(!empty($firstname) || !empty($lastname)||!empty($address)|| !empty($phone)|| !empty($email) || !empty($departuredate) || !empty($arrivaldate) || !empty($location) || !empty($msg))
{
    ...
    { 
  $SELECT = "SELECT email from kathmandubooking Where email = ? Limit 1 "; 
        
   $INSERT = "INSERT Into kathmandubooking (firstname, lastname, address,phone, email, departuredate, arrivaldate, location,msg) values(?,?,?,?,?,?,?,?,?)";
    //prepared statement
        ...
        {            
            ...
            header("Location:kathmandubookingmain.php?msg1=".$msg1);
        }
        else
        {
