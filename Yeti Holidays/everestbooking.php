...
if(!empty($firstname) || !empty($lastname)||!empty($address)|| !empty($phone)|| !empty($email) || !empty($departuredate) || !empty($arrivaldate) || !empty($location) || !empty($msg))
{
    ...
    { 
  $SELECT = "SELECT email from everestbooking Where email = ? Limit 1 "; 
   $INSERT = "INSERT Into everestbooking (firstname, lastname, address,phone, email, departuredate, arrivaldate, location,msg) values(?,?,?,?,?,?,?,?,?)";
    //prepared statement
        ...
        {            
            ...
            $msg1 .="<br>You can now enjoy our services";
             header ("Location: everestbookingmain.php?msg1=".$msg1);
        }
        else
        {
